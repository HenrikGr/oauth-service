...
const { InvalidArgumentError, InvalidGrantError, InvalidRequestError, ServerError } = require('../../errors')
...
const logger = createClientLogger('tokenHandler:authorizationCodeGrant')
...
class AuthorizationCodeGrant extends AbstractGrant {
  ...
  async validateAuthorizationCode(accessTokenRequest, client) {
    logger.info('validateAuthorizationCode: ', accessTokenRequest.code)
    const now = new Date()
  ...
  validateRedirectUri(code, accessTokenRequest) {
    logger.info('validateRedirectUri: ', accessTokenRequest.redirectUri)
    if (!code.redirectUri) {
      return
    }
  ...
  async deleteAuthorizationCode(code) {
    logger.info('deleteAuthorizationCode: ', code.code)
  ...
  async execute(request, response, client) {
    try {
       ...
       * Create a token
       */
      const data = await this.createToken(client, code)
       ...
       */
      logger.info('execute: ended gracefully')
