...
class AuthorizationCodeGrant extends AbstractGrant {
   ...
   */
  async validateAuthorizationCode(client, accessTokenRequest) {
   ...
   */
  validateRedirectUri(accessTokenRequest, code) {
  ...
  async createToken(client, code) {
    debugService('createToken: ', code.authorizationCode)
    const { user, scope, authorizationCode } = code
    const accessScope = await super.validateScope(user, client, scope)
  ...
  async execute(request, response, client) {
    try {
       ...
       * @type {Object}
       */
      const code = await this.validateAuthorizationCode(client, accessTokenRequest)

      /**
       * Validate redirect uri
       */
      this.validateRedirectUri(accessTokenRequest, code)
