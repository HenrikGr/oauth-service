...
const { log, error } = require('@hgc-ab/debug-service')('oauth:tokenHandler:authorizationCodeGrant')
...
class AuthorizationCodeGrant extends AbstractGrant {
  ...
  async validateAuthorizationCode(accessTokenRequest, client) {
    log('validateAuthorizationCode: ', accessTokenRequest.code)
    const now = new Date()
  ...
  validateRedirectUri(code, accessTokenRequest) {
    log('validateRedirectUri: ', accessTokenRequest.redirectUri)
    if (!code.redirectUri) {
      return
    }
  ...
  async deleteAuthorizationCode(code) {
    log('deleteAuthorizationCode: ', code.code)
