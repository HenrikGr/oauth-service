...
const debugService = require('@hgc-ab/debug-service')('clientCredentialsGrant')
...
class ClientCredentialGrant extends AbstractGrant {
  ...
  async validateUser(client) {
    debugService('validateUser: ', client.user.username)
   ...
   */
  async createToken(user, client, accessTokenRequest) {
    debugService('createToken: ', accessTokenRequest.scope)
    const accessScope = await super.validateScope(user, client, accessTokenRequest.scope)
  ...
  async execute(request, response, client) {
    try {
       ...
       * Create a token
       */
      const data = await this.createToken(user, client, accessTokenRequest)
