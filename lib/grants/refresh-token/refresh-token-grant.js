...
const debugService = require('@hgc-ab/debug-service')('refreshTokenGrant')
...
class RefreshTokenGrant extends AbstractGrant {
   ...
   */
  async createToken(user, client, scope) {
  ...
  async execute(request, response, client) {
    try {
       ...
       * Create a new token
       */
      const data = await this.createToken(token.user, client, token.scope)
