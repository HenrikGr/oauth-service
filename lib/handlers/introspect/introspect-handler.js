...
const debugService = require('@hgc-ab/debug-service')('introspectHandler')
const {
...
class IntrospectHandler {
   ...
   *
   * @param {IntrospectRequest} iRequest - The introspect request object
   * @param {Object} client - The authenticated client
   * @returns {Promise<{Object}>}
   * @throws {InvalidGrantError|ServerError}
   */
  async validateToken(iRequest, client) {
    debugService('validateToken:', iRequest.tokenHint)
    const token =
      iRequest.tokenHint === 'access_token'
        ? await this.model.getAccessToken(iRequest.token)
        : await this.model.getRefreshToken(iRequest.token)

    if (!token) {
      throw new ServerError(`Server error: '${iRequest.tokenHint}' did not exist`)
    }

    if (!token.client) {
      throw new ServerError(
        `Server error: '${iRequest.tokenHint}' did not contain a \`client\` object`
      )
    }

    if (!token.user) {
      throw new ServerError(
        `Server error: '${iRequest.tokenHint}' did not contain a \`user\` object`
      )
    }

    if (token.client.id !== client.id) {
      throw new InvalidGrantError(`Invalid grant: '${iRequest.tokenHint}' token is invalid`)
    }
   ...
   */
  composeIntrospectResult(iRequest, token) {
    debugService('composeIntrospectResult:', iRequest.tokenHint)
    const now = new Date()
    const expiresAt =
      iRequest.tokenHint === 'access_token'
   ...
   *
   * Execution steps
   * - create an introspect request object and validate the requested parameters
   * - authenticate the client
   * - validate the token
   * - compose introspection result
   * - create an introspect response object and update the response object
   *
  ...
  async execute(request, response) {
    try {
       ...
       */
      const iRequest = new IntrospectRequest(request, this.isClientSecretRequired)

      /**
       * Authenticate client
       */
      const client = await this.authenticateClient(iRequest.credentials)

      /**
       * Validate token
       */
      const token = await this.validateToken(iRequest, client)

      /**
       * Compose introspection result
       */
      const result = this.composeIntrospectResult(iRequest, token)
