...
const debugService = require('@hgc-ab/debug-service')('middleware:introspectHandler')
...
class IntrospectHandler {
  ...
  async verifyToken(introspectRequest, client) {
    debugService('verifyToken:', introspectRequest.tokenHint)
    const token =
  ...
  composeIntrospectResult(introspectRequest, token) {
    debugService('composeIntrospectResult:', introspectRequest.tokenHint)
