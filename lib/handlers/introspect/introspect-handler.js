...
const logger = createClientLogger('introspectHandler')
...
class IntrospectHandler {
  ...
  async verifyToken(introspectRequest, client) {
    logger.info('verifyToken:', introspectRequest.tokenHint)
    const token =
  ...
  composeIntrospectResult(introspectRequest, token) {
    logger.info('composeIntrospectResult:', introspectRequest.tokenHint)
