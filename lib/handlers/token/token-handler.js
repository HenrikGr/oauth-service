...
const debugService = require('@hgc-ab/debug-service')('tokenHandler')
const {
...
class TokenHandler {
  ...
  async authenticateClient(tokenRequest) {
    debugService('authenticateClient: ', tokenRequest.grantType)
   ...
   */
  async processGrant(tokenRequest, response, client) {
    const GrantType = this.grantTypes[tokenRequest.grantType]
    ...
    return new GrantType(this.model, options).execute(tokenRequest.request, response, client)
  }
  ...
  async execute(request, response) {
    try {
       ...
       * @type {Object}
       */
      const accessToken = await this.processGrant(tokenRequest, response, client)
