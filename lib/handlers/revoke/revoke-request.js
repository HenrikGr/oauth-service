...
/**
 * Provides a logical representation of an revoke request object
 *
 * @class
...
class RevokeRequest {
  /**
   * Create an RevokeRequest object
   *
  ...
  constructor(request, isClientSecretRequired = true) {
     ...
     */
    const { token, token_type_hint: tokenHint } = request.body

    if (!tokenHint && !token) {
      throw new InvalidRequestError('Missing parameter: `token_type_hint` and `token`')
    }
    ...
    if (!tokenHint) {
      throw new InvalidRequestError('Missing parameter: `token_type_hint`')
    }

    if (!(tokenHint === 'access_token' || tokenHint === 'refresh_token')) {
      throw new UnsupportedTokenTypeError('Invalid parameter: `token_type_hint`')
    }
