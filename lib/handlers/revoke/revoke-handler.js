...
const { log, error } = require('@hgc-ab/debug-service')('oauth:revokeHandler')
...
class RevokeHandler {
  ...
  async verifyToken(revokeRequest, client) {
    log('verifyToken:', revokeRequest.tokenHint)
    const token =
  ...
  async invalidateToken(revokeRequest, token) {
    log('invalidateToken: ', revokeRequest.tokenHint)
  ...
  async execute(request, response) {
    try {
      ...
      log(`Revoking ${revokeRequest.tokenHint}: ${revokeRequest.token}`)
