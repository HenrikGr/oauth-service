...
const logger = createClientLogger('revokeHandler')
...
class RevokeHandler {
  ...
  async authenticateClient(credentials) {
    logger.info('authenticateClient:', credentials.clientId)
  ...
  async verifyToken(revokeRequest, client) {
    logger.info('verifyToken:', revokeRequest.tokenHint)
    const token =
  ...
  async invalidateToken(revokeRequest, token) {
    logger.info('invalidateToken: ', revokeRequest.tokenHint)
  ...
  async execute(request, response) {
    try {
      ...
      logger.info(`Revoking ${revokeRequest.tokenHint}: ${revokeRequest.token}`)
