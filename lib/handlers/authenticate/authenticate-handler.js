...
const logger = createClientLogger('authenticateHandler')
...
class AuthenticateHandler {
  ...
  async validateAccessToken(authenticateRequest) {
    logger.info('validateAccessToken:', authenticateRequest.token)
    const now = new Date()
  ...
  async verifyAccessTokenScope(accessToken) {
    logger.info('verifyAccessTokenScope:', accessToken.scope, this.scope)
  ...
  async execute(request, response) {
    try {
      ...
      logger.error('execute: ', e.name, e.message)
