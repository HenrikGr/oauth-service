...
const { InsufficientScopeError, InvalidArgumentError, InvalidTokenError, ServerError } = require('../../errors')
...
const debugService = require('@hgc-ab/debug-service')('middleware:authenticateHandler')
...
class AuthenticateHandler {
   ...
   *
   * @param {AuthenticateRequest} authenticateRequest - Authenticate request instance
   * @return {Promise<{Object}>} - The access token from the data model
   * @throws {InvalidTokenError|ServerError}
   */
  async validateAccessToken(authenticateRequest) {
    debugService('validateAccessToken:', authenticateRequest.token)
    const now = new Date()
    const accessToken = await this.model.getAccessToken(authenticateRequest.token)
    if (!accessToken) {
  ...
  async verifyAccessTokenScope(accessToken) {
    debugService('verifyAccessTokenScope:', accessToken.scope, this.scope)
  ...
  async execute(request, response) {
    try {
       ...
       */
      const authenticateRequest = new AuthenticateRequest(request, this.allowBearerTokensInQueryString)

      /**
       * Validate access token
       * @type {Object}
       */
      const accessToken = await this.validateAccessToken(authenticateRequest)
