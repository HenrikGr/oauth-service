...
const logger = createClientLogger('authorizationHandler')
...
class AuthorizeHandler {
  ...
  async authenticateResourceOwner(request, response) {
    logger.info('authenticateResourceOwner')
  ...
  async validateClient(authorizationRequest) {
    logger.info('validateClient:', authorizationRequest.clientId)
   ...
   */
  async validateScope(client, user, authorizationRequest) {
    logger.info('validateScope:', authorizationRequest.scope)
    if (this.model.validateScope) {
      const validScope = await this.model.validateScope(client, user, authorizationRequest.scope)
      if (!validScope) {
  ...
  async execute(request, response) {
    try {
       ...
       * @type {String|string|*}
       */
      const validScope = await this.validateScope(client, user, authorizationRequest)
